...
const HotList = ({ list, hotClick }) => (
  <div className={styles.hotList}>
    {list.map((item, index) => {
      return (
        <span key={index} onClick={hotClick}>
          {item}
        </span>
      );
    })}
              ...
              <li key={index} onClick={() => this.handleClick(item, index)}>
                <div className={styles.musicBox}>
                  <img
                    ...
                    alt={item.title}
                    className={
                      this.state.currentIndex === index ? styles.rotateGo : ''
                    }
                  />
          ...
          {this.state.musicUrl && (
            ...
            >
              {this.state.currentIndex !== 0 && (
                <IconUp prevMusic={this.prevClick} />
              )}
              <audio
                ...
                controls
                loop
                ref={c => (this.audioEle = c)}
              />
              {this.state.currentIndex !== this.state.dataSorce.length - 1 && (
                <IconDown nextMusic={this.nextClick} />
              )}
            </div>
          )}
        </div>
